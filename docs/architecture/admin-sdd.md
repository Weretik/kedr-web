...
раскрывает только публичный API через свой `src/index.ts` и при необходимости
через `src/lib/index.ts`. Импорты во внутренние файлы другого домена запрещены.
...
- `routes` содержит конфигурацию маршрутов домена;
...
`shared` содержит только нейтральный, не-доменный код:

- `ui` — визуальные примитивы и дизайн-система;
- `api-client` — базовый HTTP-клиент, interceptors и общая обработка transport-ошибок;
- `contracts` — общие transport-контракты, не принадлежащие одному домену;
- `config` — типизированная конфигурация окружения;
- `util` — чистые общие утилиты;
- `testing` — фабрики, render helpers и test doubles.

Запрещено переносить в `shared` код, который пока нужен одному домену.
...
```text
app -> core | feature | shared
core -> shared/*
feature -> ui | model | data-access | shared/*
data-access -> model | shared/api-client | shared/contracts | shared/util
ui -> model | shared/ui | shared/util
model -> shared/contracts | shared/util
shared/* -> shared/* только при отсутствии циклов
```
...
| ------------- | --------------------------------------------- |
| `app`         | `core`, `feature`, `shared`                   |
| `feature`     | `data-access`, `model`, domain `ui`, `shared` |
| `data-access` | `model`, `api-client`, `contracts`, `util`    |
| domain `ui`   | `model`, `shared/ui`, `shared/util`           |
| `model`       | `shared/contracts`, `shared/util`             |
| `shared`      | только другие `shared`-библиотеки             |

Дополнительные ограничения:

- домен не импортирует внутренности другого домена;
- `shared` не зависит от библиотек `admin`;
...
`["scope:admin", "domain:products", "type:data-access"]`, а для общего UI —
`["scope:shared", "domain:shared", "type:ui"]`. Значения
...
import { ProductsPage } from '@admin/products/feature/src/lib/pages/ProductsPage';
```
...
type:feature     -> type:data-access | type:model | type:ui | shared
type:data-access -> type:model | api-client | contracts | util
type:ui          -> type:model | shared/ui | shared/util
type:model       -> contracts | shared/util
```
...
- Общий RTK Query API находится в `@shared/api-client`: `baseApi`, `baseQuery`,
  подготовка заголовков, обновление токена и глобальная обработка API-ошибок.
...
libs/admin/products/feature/
└── src/lib/state/products.slice.ts
```
...
`FormError`) размещаются в `@shared/ui`. Они не содержат бизнес-правил и не
...
1. Библиотека и правила server state/cache.
2. Библиотека форм и схема валидации.
3. Единый формат ошибок API и уведомлений.
4. Стратегия прав доступа на уровне маршрутов и действий.
5. Правила i18n для Admin.
